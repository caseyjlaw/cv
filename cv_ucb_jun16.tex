%"Casey Law'c CV includes "select publications" and "select invited talks."  Please ask him to provide a CV that includes all publications, listed by category (refereed, non-refereed, etc.). Each category should be numbered. It's preferable if the first publication is #1 and subsequent publications are numbered going up from there.  It would be great if he listed all talks also.  In effect, for the merit reviews, he should provide a comprehensive CV, and then update it going forward for future reviews.  This allows reviewers to cross-reference the current merit case CV with previous cases and clearly highlights recent activity". 
...
\textit{Radio interferometry}, fast transients, multiwavelength astronomy, surveys, neutron stars, intergalactic and interstellar media, big data, data science, Python, polarimetry, X-ray astronomy, star clusters
...
\item[] \textit{Student Advising}
    ...
    \begin{innerlist}
      ...
      \item Undergraduates \textbf{Jun Tan}, \textbf{Yawen Sun}, \textbf{Kyle Blanchard}, and \textbf{Phillip Sells} \\
        Students have worked with the NERSC supercomputer, AWS cloud computing, and radio interferometric search software and data analysis.
        Primary adviser: Geoff Bower and Carl Heiles, 2012 -- present.
     \end{innerlist}
\end{outerlist}
...
\begin{bibsection}
  ...
  \item {\bf PI: Real-time, commensal transient detection at the VLA (``realfast'' project), NSF, Advanced Technology and Instrumentation grant, awarded 2016.}
...
%\begin{bibsection}
\begin{enumerate}
    \item {\bf ``New methods to constrain the radio transient rate: results from a survey of four fields with LOFAR'', Carbone, D. et al 2016, MNRAS, 459, 3161}

    \item {\bf ``Low-radio-frequency eclipses of the redback pulsar J2215+5135 observed in the image plane with LOFAR'', Broderick, J. W. et al 2016, MNRAS, 459, 2681}

    \item {\bf ``LOFAR MSSS: detection of a low-frequency radio transient in 400 h of monitoring of the North Celestial Pole'', Stewart et al 2016, MNRAS, 46, 2321}

    \item {\bf ``The LOFAR Multifrequency Snapshot Sky Survey (MSSS). I. Survey description and first results'', Heald et al 2016, A\&A, 582, 123}

    \item {\bf ``A Millisecond Interferometric Search for Fast Radio Bursts with the Very Large Array'', Law, C. J. et al 2015, ApJ, 807, 16}

    \item {\bf ``The LOFAR Transients Pipeline'', Swinbank, J. D. et al, 2015, A\&C 11, 25}

    \item {\bf ``Pulsar polarisation below 200 MHz: Average profiles and propagation effects'', Noutsos, A. et al 2015, A\&A, 576, 41}

    \item {\bf ``ALMA and VLA measurements of frequency-dependent time lags in Sagittarius A*: evidence for a relativistic outflow'' Brinkerink, C. et al 2015, A\&A 576, 41}
...
\begin{enumerate}
    \item {\bf ``Science with the VLA Sky Survey (VLASS)'' Murphy et al 2015, AAS 225}

    \item {\bf ``Technical Implementation Plan for the VLA Sky Survey (VLASS)'' Myers et al, 2015, AAS 225}

    \item {\bf ``A Survey for Cosmological Millisecond Radio Transients with the Very Large Array'' Law et al, 2014, AAS 224}
\end{enumerate}

\section{Software}
\begin{enumerate}
    \item {\bf ``tpipe: Searching radio interferometry data for fast, dispersed transients'', Law, C. J. 2016, ASCL, 03012}
\end{enumerate}
...
%\begin{bibsection}
\begin{enumerate}
    \item {\bf ``realfast: Commensal Fast Transient Surveys with the VLA'', Seminar at NRAO, Socorro, NM, April 2016}

    \item {\bf ``Real-Time Commensal Fast Transient Searches with the VLA'' at "Hotwiring the Transient Universe IV'', Santa Barbara, CA, May 2015}

    \item {\bf ``Searching for Fast Radio Transients at 1 Terabyte per hour'', Seminar at NRAO Socorro, NM, March 2015}
